    ...
    <div className="bg-white flex shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded-lg w-full sm:w-[330px] ">
      <Link to={`/listing/${listing._id}`}>
        ...
        <div className="p-3 flex flex-col gap-2 w-full">
          <p className="truncate text-lg font-serif  text-primary">
            {listing?.title}
          </p>
          <div className="flex items-center gap-1">
            <MdLocationOn className="h-4 w-4 text-secondary" />
            <p className="text-sm text-secondary truncate w-full font-sans">
              {listing?.address}
            </p>
          </div>

          <p className="text-sm text-secondary line-clamp-2 font-sans ">
            {listing?.description}
          </p>
          <p className="text-secondary mt-2 font-serif   text-xs flex gap-4">
            Rs{" "}
            {listing?.offer
              ? (
          ...
          <div className="text-[#B8615F] flex gap-2">
            ...
            <div className="font-bold text-xs flex gap-1">
              <MdBathtub className="text-sm md-1" />
              {listing?.bathrooms > 1
